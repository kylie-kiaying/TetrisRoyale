          ...
          {table.getHeaderGroups().map((headerGroup) => (
            <TableRow key={headerGroup.id}>
              {headerGroup.headers.map((header) => (
                <TableHead
                  key={header.id}
                  className="border-b px-6 py-3 font-semibold text-white"
                >
              ...
              <React.Fragment key={row.id}>
                <TableRow>
                  {row.getVisibleCells().map((cell) => (
                    <TableCell
                      key={cell.id}
                      className="border-b border-gray-600 px-6 py-4 text-gray-300"
                    >
                      {flexRender(
                    ...
                    >
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="border-b border-gray-700 py-2">
                          ...
                          <p className="text-lg">
                            {row.original.finesse_percentage}
                          </p>
                        </div>
                        <div className="border-b border-gray-700 py-2">
                          <p className="text-sm font-semibold">Lines Cleared</p>
                          <p className="text-lg">
                            {row.original.lines_cleared}
                          </p>
                        </div>
                      </div>
                    </TableCell>
