...
export const fetchAdminTournamentPlayableMatches = async (tournament_id) => {
    try {
        const response = await axios.get(`http://localhost:8004/matchmaking/tournaments/${tournament_id}/matches`);
        if (response.status === 200) {
            return response.data;
        } else {
...
export const startTournament = async (tournament) => {
    try {
        ...
        const pairPlayersResponse = await axios.post(`http://localhost:8004/matchmaking/tournaments/${tournament.tournament_id}/pair`);
        if (pairPlayersResponse.status == 200) {
          const response = await axios.put(`http://localhost:8003/tournaments/${tournament.tournament_id}`, {
          ...
          if (response.status != 200) {
            ...
            for (const pair of pairPlayersResponse.data) {
              const matchResponse = await axios.post(`http://localhost:8004/matchmaking`, {
...
export const setPlayerAsWinner = async (match_id, winner_id) => {
    try {
        const response = await axios.put(`http://localhost:8004/matchmaking/${match_id}/results`, {
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      const registrationPromises = player_ids.map(player_id => 
        axios.post(`http://localhost:8003/tournaments/${tournament_id}/register`, {
          id: tournament_id,
          player_id: player_id,
        })
      );
      ...
      const allSuccessful = registrationResponses.every(res => res.status === 200);
      ...
      console.error('Failed to spawn tournament and participants:', response.statusText);
